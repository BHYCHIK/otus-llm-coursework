...
sem = asyncio.Semaphore(int(os.getenv("MAX_PARALLEL", "10")))  # лимит одновременных анализов
...
async def analyze_review(review: str = Form(..., description="Review from marketplace"),
                   product_id: int = Form(..., description="Product ID")):

    async with sem:
        result = await review_analyzer.analyze(review)
